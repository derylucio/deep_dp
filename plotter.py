...
COLORS = ['#496d64', '#6d4952', '#a8baca','#cab8a8', '#babfb7', '#ebe5ef', '#3d0062', '#256200', \
 ...
 '#e6e6fa','#2c2349', '#322044', '#222d4a', '#676767' ,'#4c0112' ,'#4c0000' ,'#800000','#aaadbd' ,'#9fadc0','#778ba5',\
  '#bdd2f8', '#bad61f', '#4522f3', '#f87d99', '#647285', '#070707', '#707070', '#777777', '#676767', '#196419', '#686472', '#191919' ,'#131313']
...
def plot(arr, save_dir, xlabel, ylabel, name, epoch):
	...
	plt.ylabel(ylabel)
	plt.tight_layout()
	plt.savefig(save_dir + "/" + name + str(epoch) + ".jpg")
	plt.close()
...
def plot_results_vid(save_dir, ys, preds, vid_kl, vid_recon, vid, epoch):
    plot(save_dir, vid_kl, "frame", "kl-loss", vid + "_kl", epoch)
    plot(save_dir, vid_recon, "frame", "recon-loss", vid + "_recon", epoch)
...
def visualize(save_dir, epoch, vid, true_seg_labels, true_segs, prop_seg_labels, prop_segs):
    ...
    for ind, segmentation in enumerate([true_segs, prop_segs]):
        for seg_ind, segment in enumerate(segmentation):
            start, end = segment
            plt.barh(ind, end - start, height=1, align = 'center', left = start, color = COLORS[labels[ind][seg_ind] % len(COLORS)])
