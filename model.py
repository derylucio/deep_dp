...
class MultinomCell(nn.Module):
    def __init__(self, feature_dim, batch_dim, hidden_size, num_factors, gru_cell=True):
        super(MultinomCell, self).__init__()
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.latent_factors = Variable(
            torch.rand(num_factors, latent_dim))  # TODO: change from random uniform (0, 1) to something else !
        if is_cuda:
            self.latent_factors = self.latent_factors.cuda()
...
class DeepDP(nn.Module):
    ...
    def getVGG(self, finetune=False):
        ...
        pretrained_state = {k: v for k, v in pretrained_state.iteritems() if k in my_vgg_state}
    ...
    def rewind(self):
        self.prev_x = Variable(torch.zeros(self.batch_dim, self.feature_dim), requires_grad=False)
        if torch.cuda.is_available():
            self.prev_x = self.prev_x.cuda()
